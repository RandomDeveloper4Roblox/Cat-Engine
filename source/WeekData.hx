...
class WeekData {
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false)
	{
		...
		var aList:Array<String> = CoolUtil.coolTextFile(Paths.getPreloadPath('weeks/weekList.txt'));
		for (i in 0...aList.length) {
			for (j in 0...directories.length) {
				var fileToCheck:String = directories[j] + 'weeks/' + aList[i] + '.json';
				if(!weeksLoaded.exists(aList[i])) {
					var week:WeekFile = getWeekFile(fileToCheck);
					if(week != null) {
						var weekFile:WeekData = new WeekData(week, aList[i]);
						...
						if(weekFile != null && (isStoryMode == null || (isStoryMode && !weekFile.hideStoryMode) || (!isStoryMode && !weekFile.hideFreeplay))) {
							weeksLoaded.set(aList[i], weekFile);
							weeksList.push(aList[i]);
						}
					}
				}
			}
